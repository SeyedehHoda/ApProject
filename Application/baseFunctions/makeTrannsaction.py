...
def make_transaction(user_id):
    ...
    if origin_account_id == 'back_code':
        return
    origin_account = handle_query(f'SELECT FROM bank_account WHERE id=={origin_account_id} AND is_active==true')[1][0]
    ...
    if destination_account_id == 'back_code':
        return
    destination_account = handle_query(f'SELECT FROM bank_account WHERE id=={destination_account_id} AND is_active==true')[1][0]
    ...
    if money > int(origin_account['money']):
        ...
        menu_user()
        return
    values1 = str(origin_account_id) + ',' + str(destination_account_id) + ',null,' + str(money)
    res1 = handle_query(f'INSERT INTO transactions VALUES ({values1})')
    values2 = origin_account['user_id'] + ',' + origin_account['password'] \
              + ',' + origin_account['account_number'] + ',' + str(int(origin_account['money']) - money) + ',true'
